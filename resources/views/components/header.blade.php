        ...
        <div class="dropdown d-inline-block">
            <button type="button" class="btn header-item user text-start d-flex align-items-center"
                ...
                <span class="ms-2 d-none d-sm-block user-item-desc">
                    <span class="user-name">{{ Auth::user()->guru->nama ?? '' }}</span>
                    <span class="user-sub-title">{{ Auth::user()->guru->kode_guru ?? '' }}</span>
                </span>
            </button>
            <div class="dropdown-menu dropdown-menu-end pt-0">
                <div class="p-3 bg-primary border-bottom">
                    <h6 class="mb-0 text-white">{{ Auth::user()->guru->nama ?? '' }}</h6>
                    <p class="mb-0 font-size-11 text-white-50 fw-semibold">{{ Auth::user()->guru->kode_guru ?? '' }}</p>
                </div>
